...
export default class HomeScreen
    ...
    login = async () => {
        try {
            var rs = await ZaloKit.login(ZaloKit.Constants.AUTH_VIA_APP_OR_WEB)
            console.log('rs43243687898');
            console.log(rs);
            this.setState({
                loginStatus: true, code: rs
            })
        } catch (error) {
            console.log(error)
    ...
    getUserProfile = async () => {
        try {
            ...
            console.log('userProfile3132');
            console.log(userProfile)

            console.log('rs123123132');
            console.log(this.state.code);
            let token = await fetch(`https://oauth.zaloapp.com/v3/access_token?app_id=2874211114250861487&app_secret=TyDEUXE6xCLIKZ4BVGS5&code=${this.state.code}`)
            let jsonx = await token.json()
            console.log('jsonx3312321');
            console.log(jsonx)
    ...
    getFriendListInvitable = async () => {
        try {
            const friends = await ZaloKit.getFriendListInvitable(1, 50)
            console.log(friends)
        } catch (error) {
            console.log(error)
        }
    ...
    sendMessageToFriend = async () => {
        try {
            const friendId = '4627108529344051684'
            const link = 'https://developers.zalo.me/'
            const message = 'Hello World'
